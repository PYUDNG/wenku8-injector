// 书签版
javascript:(function(){const KEY_LOCALCDN='LOCAL-CDN';const KEY_LOCALCDN_VERSION='version';const VALUE_LOCALCDN_VERSION='0.1';const KEY_POLYFILL='wenku8-plus-injecter';GM_PolyFill();const url='https://greasyfork.org/scripts/416310-%E8%BD%BB%E5%B0%8F%E8%AF%B4%E6%96%87%E5%BA%93/code/%E8%BD%BB%E5%B0%8F%E8%AF%B4%E6%96%87%E5%BA%93+.user.js';loadJSPlus(url,function(success){if(success){console.log('轻小说文库+ loaded');}else{console.log('轻小说文库+ load failed');}});function loadJSPlus(url,callback,oDoc=document,maxRetry=3){const fn=callback||function(){};const localCDN=GM_getValue(KEY_LOCALCDN,{});if(localCDN[url]){console.log('Loading js from localCDN: '+url);const js=localCDN[url];appendScript(js);fn(true);return;};console.log('Loading js from web: '+url);loadJSPlus.retryCount=loadJSPlus.retryCount!==undefined?loadJSPlus.retryCount:0;GM_xmlhttpRequest({method:'GET',url:url,responseType:'text',onload:function(e){if(e.status===200){const js=e.responseText;localCDN[url]=js;localCDN[KEY_LOCALCDN_VERSION]=VALUE_LOCALCDN_VERSION;GM_setValue(KEY_LOCALCDN,localCDN);appendScript(js);fn(true);}else{retry();};},onerror:retry});function appendScript(code){const script=oDoc.createElement('script');script.type='text/javascript';script.innerHTML=code;oDoc.head.appendChild(script);};function retry(){loadJSPlus.retryCount++;if(loadJSPlus.retryCount<=maxRetry){loadJSPlus(url,callback,oDoc);}else{fn(false);};};function appendScript(code){const script=oDoc.createElement('script');script.type='text/javascript';script.innerHTML=code;oDoc.head.appendChild(script);};};function GM_PolyFill(name='default'){const GM_POLYFILL_KEY_STORAGE='GM_STORAGE_POLYFILL';const GM_POLYFILL_storage=GM_POLYFILL_getStorage();const GM_POLYFILLED={GM_setValue:true,GM_getValue:true,GM_deleteValue:true,GM_listValues:true,GM_xmlhttpRequest:true,GM_openInTab:true,GM_setClipboard:true,};GM_setValue_polyfill();GM_getValue_polyfill();GM_deleteValue_polyfill();GM_listValues_polyfill();GM_xmlhttpRequest_polyfill();GM_openInTab_polyfill();GM_setClipboard_polyfill();function GM_POLYFILL_getStorage(){let gstorage=localStorage.getItem(GM_POLYFILL_KEY_STORAGE);gstorage=gstorage?JSON.parse(gstorage):{};let storage=gstorage[name]?gstorage[name]:{};return storage;};function GM_POLYFILL_saveStorage(){let gstorage=localStorage.getItem(GM_POLYFILL_KEY_STORAGE);gstorage=gstorage?JSON.parse(gstorage):{};gstorage[name]=GM_POLYFILL_storage;localStorage.setItem(GM_POLYFILL_KEY_STORAGE,JSON.stringify(gstorage));};function GM_setValue_polyfill(){typeof(GM_setValue)==='function'?GM_POLYFILLED.GM_setValue=false:window.GM_setValue=PF_GM_setValue;;function PF_GM_setValue(name,value){name=String(name);GM_POLYFILL_storage[name]=value;GM_POLYFILL_saveStorage();};};function GM_getValue_polyfill(){typeof(GM_getValue)==='function'?GM_POLYFILLED.GM_getValue=false:window.GM_getValue=PF_GM_getValue;function PF_GM_getValue(name,defaultValue){name=String(name);if(GM_POLYFILL_storage.hasOwnProperty(name)){return GM_POLYFILL_storage[name];}else{return defaultValue;};};};function GM_deleteValue_polyfill(){typeof(GM_deleteValue)==='function'?GM_POLYFILLED.GM_deleteValue=false:window.GM_deleteValue=PF_GM_deleteValue;function PF_GM_deleteValue(name){name=String(name);if(GM_POLYFILL_storage.hasOwnProperty(name)){delete GM_POLYFILL_storage[name];GM_POLYFILL_saveStorage();};};};function GM_listValues_polyfill(){typeof(GM_listValues)==='function'?GM_POLYFILLED.GM_listValues=false:window.GM_listValues=PF_GM_listValues;function PF_GM_listValues(){return Object.keys(GM_POLYFILL_storage);};};function GM_xmlhttpRequest_polyfill(){typeof(GM_xmlhttpRequest)==='function'?GM_POLYFILLED.GM_xmlhttpRequest=false:window.GM_xmlhttpRequest=PF_GM_xmlhttpRequest;function PF_GM_xmlhttpRequest(details){const xhr=new XMLHttpRequest();const openArgs=[details.method,details.url,true];if(details.user&&details.password){openArgs.push(details.user);openArgs.push(details.password);}xhr.open.apply(xhr,openArgs);if(details.headers){for(const key of Object.keys(details.headers)){xhr.setRequestHeader(key,details.headers[key]);};};details.cookie?xhr.setRequestHeader('cookie',details.cookie):function(){};details.anonymous?xhr.setRequestHeader('cookie',''):function(){};xhr.timeout=details.timeout;xhr.responseType=details.responseType;details.overrideMimeType?xhr.overrideMimeType(details.overrideMimeType):function(){};xhr.onabort=details.onabort;xhr.onerror=details.onerror;xhr.onloadstart=details.onloadstart;xhr.onprogress=details.onprogress;xhr.onreadystatechange=details.onreadystatechange;xhr.ontimeout=details.ontimeout;xhr.onload=function(e){const response={readyState:xhr.readyState,status:xhr.status,statusText:xhr.statusText,responseHeaders:xhr.getAllResponseHeaders(),response:xhr.response};(details.responseType===''||details.responseType==='text')?(response.responseText=xhr.responseText):function(){};(details.responseType===''||details.responseType==='document')?(response.responseXML=xhr.responseXML):function(){};details.onload(response);};details.data?xhr.send(details.data):xhr.send();return{abort:xhr.abort};};};function GM_openInTab_polyfill(){typeof(GM_openInTab)==='function'?GM_POLYFILLED.GM_openInTab=false:window.GM_openInTab=PF_GM_openInTab;function PF_GM_openInTab(url){window.open(url);};};function GM_setClipboard_polyfill(){typeof(GM_setClipboard)==='function'?GM_POLYFILLED.GM_setClipboard=false:window.GM_setClipboard=PF_GM_setClipboard;function PF_GM_setClipboard(text){const newInput=document.createElement('textarea');document.body.appendChild(newInput);newInput.value=text;newInput.select();document.execCommand('copy');document.body.removeChild(newInput);};};return GM_POLYFILLED;};})();
...
(function () {
  ...
  function dealDocument(oDom=document) {
    ...
    function openWithInjector(strUrl, strWindowName, strWindowFeatures) {
      ...
      loadJSPlus(loader_url, function(success) {
        if (success) {
          DoLog(['wenku8+ loader loaded for', oDom]);
        } else {
          DoLog(['wenku8+ loader load failed on', oDom]);
        }
      }, oDom);
      return newTab;
    }
...
// 旧版书签v2
javascript:(function(){const KEY_LOCALCDN='LOCAL-CDN';const KEY_LOCALCDN_VERSION='version';const VALUE_LOCALCDN_VERSION='0.1';GM_PolyFill();const url='https://greasyfork.org/scripts/416310-%E8%BD%BB%E5%B0%8F%E8%AF%B4%E6%96%87%E5%BA%93/code/%E8%BD%BB%E5%B0%8F%E8%AF%B4%E6%96%87%E5%BA%93+.user.js';loadJSwithlocalCDN(url,function(){console.log('轻小说文库+ loaded');});function loadJSwithlocalCDN(url,callback,oDoc=document){const fn=callback||function(){};const localCDN=GM_getValue(KEY_LOCALCDN,{});if(localCDN[url]){console.log('Loading js from localCDN: '+url);const js=localCDN[url];appendScript(js);fn();return;};console.log('Loading js from web: '+url);GM_xmlhttpRequest({method:'GET',url:url,responseType:'text',onload:function(e){const js=e.responseText;localCDN[url]=js;localCDN[KEY_LOCALCDN_VERSION]=VALUE_LOCALCDN_VERSION;GM_setValue(KEY_LOCALCDN,localCDN);appendScript(js);fn();}});function appendScript(code){const script=oDoc.createElement('script');script.type='text/javascript';script.innerHTML=code;oDoc.head.appendChild(script);};};function GM_PolyFill(){const GM_POLYFILL_KEY_STORAGE='GM_STORAGE_POLYFILL';const GM_POLYFILL_storage=GM_POLYFILL_getStorage();const GM_POLYFILLED={GM_setValue:true,GM_getValue:true,GM_deleteValue:true,GM_listValues:true,GM_xmlhttpRequest:true,GM_openInTab:true,GM_setClipboard:true,};GM_setValue_polyfill();GM_getValue_polyfill();GM_deleteValue_polyfill();GM_listValues_polyfill();GM_xmlhttpRequest_polyfill();GM_openInTab_polyfill();GM_setClipboard_polyfill();function GM_POLYFILL_getStorage(){let storage=localStorage.getItem(GM_POLYFILL_KEY_STORAGE);storage=storage?JSON.parse(storage):{};return storage;};function GM_POLYFILL_saveStorage(){localStorage.setItem(GM_POLYFILL_KEY_STORAGE,JSON.stringify(GM_POLYFILL_storage));};function GM_setValue_polyfill(){typeof(GM_setValue)==='function'?GM_POLYFILLED.GM_setValue=false:window.GM_setValue=PF_GM_setValue;;function PF_GM_setValue(name,value){name=String(name);GM_POLYFILL_storage[name]=value;GM_POLYFILL_saveStorage();};};function GM_getValue_polyfill(){typeof(GM_getValue)==='function'?GM_POLYFILLED.GM_getValue=false:window.GM_getValue=PF_GM_getValue;function PF_GM_getValue(name,defaultValue){name=String(name);if(GM_POLYFILL_storage.hasOwnProperty(name)){return GM_POLYFILL_storage[name];}else{return defaultValue;};};};function GM_deleteValue_polyfill(){typeof(GM_deleteValue)==='function'?GM_POLYFILLED.GM_deleteValue=false:window.GM_deleteValue=PF_GM_deleteValue;function PF_GM_deleteValue(name){name=String(name);if(GM_POLYFILL_storage.hasOwnProperty(name)){delete GM_POLYFILL_storage[name];GM_POLYFILL_saveStorage();};};};function GM_listValues_polyfill(){typeof(GM_listValues)==='function'?GM_POLYFILLED.GM_listValues=false:window.GM_listValues=PF_GM_listValues;function PF_GM_listValues(){return Object.keys(GM_POLYFILL_storage);};};function GM_xmlhttpRequest_polyfill(){typeof(GM_xmlhttpRequest)==='function'?GM_POLYFILLED.GM_xmlhttpRequest=false:window.GM_xmlhttpRequest=PF_GM_xmlhttpRequest;function PF_GM_xmlhttpRequest(details){const xhr=new XMLHttpRequest();const openArgs=[details.method,details.url,true];if(details.user&&details.password){openArgs.push(details.user);openArgs.push(details.password);}xhr.open.apply(xhr,openArgs);if(details.headers){for(const key of Object.keys(details.headers)){xhr.setRequestHeader(key,details.headers[key]);};};details.cookie?xhr.setRequestHeader('cookie',details.cookie):function(){};details.anonymous?xhr.setRequestHeader('cookie',''):function(){};xhr.timeout=details.timeout;xhr.responseType=details.responseType;details.overrideMimeType?xhr.overrideMimeType(details.overrideMimeType):function(){};xhr.onabort=details.onabort;xhr.onerror=details.onerror;xhr.onloadstart=details.onloadstart;xhr.onprogress=details.onprogress;xhr.onreadystatechange=details.onreadystatechange;xhr.ontimeout=details.ontimeout;xhr.onload=function(e){const response={readyState:xhr.readyState,status:xhr.status,statusText:xhr.statusText,responseHeaders:xhr.getAllResponseHeaders(),response:xhr.response};(details.responseType===''||details.responseType==='text')?(response.responseText=xhr.responseText):function(){};(details.responseType===''||details.responseType==='document')?(response.responseXML=xhr.responseXML):function(){};details.onload(response);};details.data?xhr.send(details.data):xhr.send();return{abort:xhr.abort};};};function GM_openInTab_polyfill(){typeof(GM_openInTab)==='function'?GM_POLYFILLED.GM_openInTab=false:window.GM_openInTab=PF_GM_openInTab;function PF_GM_openInTab(url){window.open(url);};};function GM_setClipboard_polyfill(){typeof(GM_setClipboard)==='function'?GM_POLYFILLED.GM_setClipboard=false:window.GM_setClipboard=PF_GM_setClipboard;function PF_GM_setClipboard(text){const newInput=document.createElement('textarea');document.body.appendChild(newInput);newInput.value=text;newInput.select();document.execCommand('copy');document.body.removeChild(newInput);};};return GM_POLYFILLED;};})();

// 旧版书签
javascript:const url='https://greasyfork.org/scripts/416310-%E8%BD%BB%E5%B0%8F%E8%AF%B4%E6%96%87%E5%BA%93/code/%E8%BD%BB%E5%B0%8F%E8%AF%B4%E6%96%87%E5%BA%93+.user.js';loadJS(url,function(){console.log('轻小说文库+ loaded');});function loadJS(url,callback,oDoc=document){var script=document.createElement('script'),fn=callback||function(){};script.type='text/javascript';if(script.readyState){script.onreadystatechange=function(){if(script.readyState=='loaded'||script.readyState=='complete'){script.onreadystatechange=null;fn();};};}else{script.onload=function(){fn();};};script.src=url;oDoc.getElementsByTagName('head')[0].appendChild(script);};
